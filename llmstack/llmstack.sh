...
ROUTER_SOCK="$LLMSTACK_DIR/router.sock"
...
Per-project layout:
  ...
  Daemons are singleton (one set at a time, on ports 10101/10102 + a UDS at
  .llmstack/router.sock for tooling). Only one channel runs at any moment.
...
cmd_start() {
    ...
    if [[ ! -f "$OPENCODE_JSON" ]]; then
        if [[ -d "$WORK_DIR/.run" && -f "$WORK_DIR/.run/opencode.json" ]]; then
            cat >&2 <<EOF
[!] found legacy $WORK_DIR/.run/ but no $LLMSTACK_DIR/
    The stack was renamed from .run/ to .llmstack/ -- re-run install to
    regenerate config in the new location:

      bash llmstack.sh install
EOF
        else
            echo "[!] no .llmstack/opencode.json in $WORK_DIR -- run: bash llmstack.sh install" >&2
        fi
        exit 1
    fi
    _ensure_state_dirs
...
EOF
    ...
    if [[ "$launch_daemons" -eq 1 ]]; then
        ...
        # Stale UDS from a crashed previous run will block uvicorn's bind.
        # router.py also unlinks defensively, but be doubly safe here.
        rm -f "$ROUTER_SOCK"
        echo "[*] starting router on :10101 (+ uds $ROUTER_SOCK)"
        cd "$SRC_DIR"
        LLAMA_SWAP_URL="http://127.0.0.1:10102" \
        ROUTER_HOST="127.0.0.1" ROUTER_PORT="10101" \
        ROUTER_UDS="$ROUTER_SOCK" \
...
[OK] stack is up (channel: $channel_label).
  ...
  router.sock  $ROUTER_SOCK
               (tooling only; curl --unix-socket <path> http://x/v1/models)

Try:
  curl -s http://127.0.0.1:10101/v1/models | jq '.data[].id'
  curl -s --unix-socket $ROUTER_SOCK http://x/health | jq '.tiers'
...
cmd_stop() {
    ...
    rm -f "$ACTIVE_MARKER" "$ROUTER_SOCK"
    echo "[OK] stopped."
}
...
cmd_status() {
    ...
    if [[ -f "$OPENCODE_JSON" ]]; then
        ...
        echo "  opencode      (not generated for this work dir; run: bash llmstack.sh install)"
        # Nudge if the old .run/ layout is present -- user needs to re-run install.
        if [[ -d "$WORK_DIR/.run" ]]; then
            echo "  [!] found legacy $WORK_DIR/.run/ -- re-run 'bash llmstack.sh install'"
            echo "      to migrate state to $LLMSTACK_DIR/"
        fi
    fi

    if [[ -S "$ROUTER_SOCK" ]]; then
        echo "  router.sock   $ROUTER_SOCK"
        echo "                tooling: curl --unix-socket $ROUTER_SOCK http://x/v1/models | jq"
    fi
