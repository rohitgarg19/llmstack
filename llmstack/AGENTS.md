...
The repo is at `~/Projects/opencode/`. Source of truth is `models.ini`;
`llama-swap.yaml` and the active `opencode.json` (in `llmstack/.run/`) are
both auto-generated by `llmstack/src/gen_*` and overwritten on every
