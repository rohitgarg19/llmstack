...
def build_cmd(tier, section) -> str:
    ...
    lines: list[str] = [
        ...
        "# See UPGRADING.md. To change permanently, edit models.ini and re-run scripts/install.sh.",
    ]
    if tier.file_next:
        lines.append(
            f"# Queued upgrade target (already pre-fetched if scripts/download-models.sh has run):"
        )
        lines.append(f"#   -hff {tier.file_next}")
    lines += [
        f"-hf {tier.repo}",
        f"-hff {tier.file}",
        f"--alias {tier.name}",
...
def build_models_block(cfg) -> dict:
    tiers = load_tiers()
    out: dict = {}
    for name, tier in tiers.items():
        ...
        out[name] = {
            ...
            "metadata": {
                ...
                "size_gb": parse_size_gb(section.get("size_gb", ""), default=0.0),
                "quant": (section.get("quant") or "").strip(),
            },
        }
    return out
...
# Hand edits will be overwritten on the next `bash scripts/install.sh`.
