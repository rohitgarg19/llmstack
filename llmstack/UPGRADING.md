...
| `llmstack/.run/opencode.json` | AUTO-GENERATED by `src/gen_opencode_config.py`. Wires opencode's `model`, `small_model` and `agent.*` to the right tiers. Loaded by opencode via the `OPENCODE_CONFIG` env var that `llmstack.sh start`/`shell` exports — your global `~/.config/opencode/opencode.json` is intentionally left alone. Regenerate via `bash llmstack.sh install`. | Auto - don't hand-edit. |
...
Under the hood, `start.sh --next` runs
`gen_llama_swap_yaml.py --use-next` to write an ephemeral sidecar at
`llmstack/.run/llama-swap.next.yaml` and points llama-swap at that file.
