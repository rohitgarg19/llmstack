...
- **`bash llmstack.sh download` uses `llama-cli`** so the cache stays
...
| `llmstack/llama-swap.yaml` | AUTO-GENERATED by `src/gen_llama_swap_yaml.py`. Determines what `llama-server` actually loads. Regenerate via `bash llmstack.sh install`. | Auto - don't hand-edit. |
| `<work-dir>/.run/opencode.json` | AUTO-GENERATED by `src/gen_opencode_config.py`. `<work-dir>` is whatever directory `llmstack.sh` was invoked from (typically `llmstack/`, but you can `cd` into any project to get a project-local config). Wires opencode's `model`, `small_model` and `agent.*` to the right tiers. Loaded by opencode via the `OPENCODE_CONFIG` env var that `llmstack.sh start`/`shell` exports — your global `~/.config/opencode/opencode.json` is intentionally left alone. Regenerate via `bash llmstack.sh install`. | Auto - don't hand-edit. |
| `<work-dir>/.run/AGENTS.md` | Copy of `llmstack/AGENTS.md` (the template), made by `install`. Loaded by opencode via the `instructions` field of `opencode.json`. Edit the source template to change defaults; edit the `.run/` copy for one-off project tweaks (will be overwritten on next `install`). | Auto - copy of template. |
| `bash llmstack.sh download` | Enumerates download targets at runtime via `python src/tiers.py --downloads` (which reads `models.ini`). | No edits — it reflects `models.ini` automatically. |

`bash llmstack.sh check` flags any DRIFT between `models.ini` (the
recommended file per tier) and `llama-swap.yaml` (the file actually
configured). If you ever see DRIFT, just re-run `bash llmstack.sh install`
to regenerate the yaml from the ini.

### Where the download list comes from

`bash llmstack.sh download` does **not** hardcode the (repo, file) tuples.
...
`models.ini` or the canonical `llama-swap.yaml`:

```bash
bash llmstack.sh stop
bash llmstack.sh start --next
bash llmstack.sh status           # shows "channel: next" + per-tier swap list
```

Under the hood, `start --next` runs
`gen_llama_swap_yaml.py --use-next` to write an ephemeral sidecar at
`<work-dir>/.run/llama-swap.next.yaml` and points llama-swap at that file.
Tiers with no `hf_file_next` are unchanged. To revert, just stop and
start again without `--next`. To make the upgrade permanent, swap
`hf_file_next` into `hf_file` in `models.ini` and re-run
...
Inside the `download` action of `llmstack/llmstack.sh`, four `_run_dl` calls map 1:1 to
...
```
0. (Optional, fast) bash llmstack.sh check
...
5. Regenerate everything from the ini:
       ...
       bash llmstack.sh install           # rewrites llama-swap.yaml + opencode.json
       bash llmstack.sh download   # picks up the new (repo, file) pair
...
7. Sanity-check via opencode (or curl) — fire a few characteristic prompts at
   ...
   one-line revert in models.ini + `bash llmstack.sh install` + restart.
```
...
bash llmstack.sh check > /tmp/before.txt
...
# 3. Edit ONE file: ../models.ini  -> the [plan] section.
#      OLD:
...
bash llmstack.sh install
bash llmstack.sh restart
bash llmstack.sh status
bash llmstack.sh check          # confirms no DRIFT vs models.ini
```

Roll back? Revert the two lines in `models.ini` and re-run
`bash llmstack.sh install && bash llmstack.sh restart`. The old GGUF is
still in the HF cache so loading it costs nothing extra.
...
bash llmstack.sh stop && bash llmstack.sh start   # bounce the router
bash llmstack.sh check                    # smoke-test PyYAML + huggingface_hub
```
...
3. Test the router:
       ...
       bash llmstack.sh stop && bash llmstack.sh start
...
downloaded by `bash llmstack.sh install-llama-swap`, which is invoked as the first
step of `bash llmstack.sh install` (and therefore also `bash llmstack.sh setup`).

```bash
bash llmstack.sh install-llama-swap           # latest, idempotent
bash llmstack.sh install-llama-swap --force   # redownload even if up-to-date
LLAMA_SWAP_VERSION=v211 \
  bash llmstack.sh install-llama-swap         # pin a specific tag
```
...
After bumping the binary, also re-run `bash llmstack.sh install` if the new
