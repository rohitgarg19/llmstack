...
generated from it. The first time you run `llmstack init` in a fresh
...
|---|---|---|
| **`<work-dir>/.llmstack/models.ini`** | The single source of truth: tier definitions (repo, file, ctx, sampler, role, …). All other artefacts derive from it. Auto-seeded from the bundled template on first `init`; gitignored after that — yours to edit per project. | **Yes — primary edit.** |
...
| `<work-dir>/.llmstack/llama-swap.yaml` | AUTO-GENERATED by `llmstack.generators.llama_swap`. Determines what `llama-server` actually loads. Regenerated fresh by `llmstack configure` (or `llmstack restart`) for the channel you're booting into. Do not regenerate with `llmstack init` — `init` only writes input files, `configure` writes the outputs. | Auto — don't hand-edit. |
| `<work-dir>/.llmstack/opencode.json` | AUTO-GENERATED by `llmstack.generators.opencode`. `<work-dir>` is whatever directory the CLI was invoked from — `cd` into any project to get a project-local config. Wires opencode's `model`, `small_model`, and `agent.*` to the right tiers. Loaded by opencode via the `OPENCODE_CONFIG` env var that the activate hook (or `llmstack start`'s fallback subshell) exports — your global `~/.config/opencode/opencode.json` is intentionally left alone. Regenerate via `llmstack configure`. | Auto — don't hand-edit. |
| `<work-dir>/.llmstack/instructions.md` | Copy of `llmstack/instructions.md` (the bundled template), made by `init`. Loaded by opencode via the `instructions` field of `opencode.json`. Edit the source template to change defaults; edit the `.llmstack/` copy for one-off project tweaks (will be overwritten on next `init --force`). | Auto — copy of template. |
...
`models.ini` and re-run `llmstack configure`.
...
5. Regenerate everything from the ini:
       ...
       llmstack configure   # rewrites opencode.json + llama-swap.yaml
       llmstack download    # picks up the new (repo, file) pair from models.ini

6. Cycle the stack:

       llmstack restart     # stop + configure + start
       llmstack status
...
7. Sanity-check via opencode (or curl) — fire a few characteristic prompts at
   ...
   one-line revert in models.ini + `llmstack configure` + `llmstack restart`.
```
...
`llmstack configure && llmstack restart`. The old GGUF is still in the HF
...
After bumping the binary, re-run `llmstack configure` only if the new
