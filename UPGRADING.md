...
| `<work-dir>/.llmstack/llama-swap.yaml` | AUTO-GENERATED by `llmstack.generators.llama_swap`. Determines what `llama-server` actually loads. Regenerate via `llmstack install`. | Auto — don't hand-edit. |
| `<work-dir>/.llmstack/opencode.json` | AUTO-GENERATED by `llmstack.generators.opencode`. `<work-dir>` is whatever directory the CLI was invoked from — `cd` into any project to get a project-local config. Wires opencode's `model`, `small_model`, and `agent.*` to the right tiers. Loaded by opencode via the `OPENCODE_CONFIG` env var that `llmstack start` / `llmstack shell` exports — your global `~/.config/opencode/opencode.json` is intentionally left alone. Regenerate via `llmstack install`. | Auto — don't hand-edit. |
...
| `llmstack download` | Enumerates download targets at runtime via `llmstack.tiers.iter_downloads` (which reads `models.ini`). | No edits — it reflects `models.ini` automatically. |
...
see DRIFT, just re-run `llmstack install` to regenerate the yaml from the
ini.
...
5. Regenerate everything from the ini:
       ...
       llmstack install     # rewrites llama-swap.yaml + opencode.json + AGENTS.md
